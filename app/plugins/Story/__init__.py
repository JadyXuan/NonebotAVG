...
@on_command('活动调试', permission=permission.PRIVATE)
async def activity_debug(session: CommandSession):
    if session.current_arg_text == '退出':  # 退出后要做的动作
        bot = session.bot
        await bot.send_group_msg(group_id=1020570636, message="测试样例")
    elif not session.state['isLoad']:  # 如果还未载入故事
...
# activity.args_parser 装饰器将函数声明为 活动调试命令的参数解析器
# 此处的命令解析器用于解析故事名
@activity_debug.args_parser
async def _(session: CommandSession):
    # 去掉消息首尾的空白字符
    stripped_arg = session.current_arg_text.strip()
